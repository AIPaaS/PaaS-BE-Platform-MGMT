...
public class PcAppImageSvcImpl implements PcAppImageSvc {
	...
	public Long saveOrUpdate(PcAppImage record) {
		...
		if (isadd) {
			...
			// BinaryUtils.checkEmpty(record.getLogMpPath(),
			// "record.logMpPath");
			// BinaryUtils.checkEmpty(record.getDataMpPath(),
			// "record.dataMpPath");
			...
			if (record.getContainerFullName() != null)
				BinaryUtils.checkEmpty(record.getContainerFullName(), "record.containerFullName");
			if (record.getDataCenterId() != null)
				BinaryUtils.checkEmpty(record.getDataCenterId(), "record.dataCenterId");
			if (record.getResCenterId() != null)
				BinaryUtils.checkEmpty(record.getResCenterId(), "record.resCenterId");
			if (record.getNetZoneId() != null)
				BinaryUtils.checkEmpty(record.getNetZoneId(), "record.netZoneId");
			if (record.getCpuCount() != null)
				BinaryUtils.checkEmpty(record.getCpuCount(), "record.cpuCount");
			if (record.getMemSize() != null)
				BinaryUtils.checkEmpty(record.getMemSize(), "record.memSize");
			if (record.getDiskSize() != null)
				BinaryUtils.checkEmpty(record.getDiskSize(), "record.diskSize");
			if (record.getInstanceCount() != null)
				BinaryUtils.checkEmpty(record.getInstanceCount(), "record.instanceCount");
			if (record.getIsSupportFlex() != null)
				BinaryUtils.checkEmpty(record.getIsSupportFlex(), "record.isSupportFlex");
			if (record.getCpuFlexUpperLimit() != null)
				BinaryUtils.checkEmpty(record.getCpuFlexUpperLimit(), "record.cpuFlexUpperLimit");
			if (record.getCpuFlexLowerLimit() != null)
				BinaryUtils.checkEmpty(record.getCpuFlexLowerLimit(), "record.cpuFlexLowerLimit");
			if (record.getMaxInstanceCount() != null)
				BinaryUtils.checkEmpty(record.getMaxInstanceCount(), "record.maxInstanceCount");
			if (record.getMinInstanceCount() != null)
				BinaryUtils.checkEmpty(record.getMinInstanceCount(), "record.minInstanceCount");
			// if(record.getLogMpPath() != null)
			// BinaryUtils.checkEmpty(record.getLogMpPath(),
			// "record.logMpPath");
			// if(record.getDataMpPath() != null)
			// BinaryUtils.checkEmpty(record.getDataMpPath(),
			// "record.dataMpPath");
	...
	public Long saveAppImageOpenService(Integer isOpen, Long appImageId, PcService svc, List<PcKvPair> params) {
		...
		if (app == null)
			throw new ServiceException(" not found app by id '" + appImg.getAppId() + "'! ");

		PcAppVersion appVno = appVnoDao.selectById(appImg.getAppVnoId());
		if (appVno == null)
			throw new ServiceException(" not found app-verion by id '" + appImg.getAppVnoId() + "'! ");
	...
	public List<AppImageSvcInfo> getAppImageCallService(Long appImageId) {
		...
		CPcAppImgSvc cdt = new CPcAppImgSvc();
		cdt.setAppImgId(appImageId);
		cdt.setSvcTypes(new Integer[] { 1, 2, 3 }); // 1=平台服务 2=外部服务 3=镜像服务
													// 4=非服务镜像
		List<PcAppImgSvc> ls = appImgSvcDao.selectList(cdt, null);
	...
	public void finishAppImageSettings(Long appImageId) {
		...
		if (cpuCount == null || cpuCount.intValue() <= 0)
			throw new ServiceException(" the appImage.cpuCount is wrong '" + cpuCount + "'! ");
		if (memSize == null || memSize.intValue() <= 0)
			throw new ServiceException(" the appImage.memSize is wrong '" + memSize + "'! ");
		if (instanceCount == null || instanceCount.intValue() <= 0)
			throw new ServiceException(" the appImage.instanceCount is wrong '" + instanceCount + "'! ");

		List<AppZoneResInfo> appresls = appSvc.queryAppNetZoneResInfo(appId, appVnoId, netZoneId);
		if (appresls.size() == 0)
			throw new ServiceException(" Don't have the resources app [" + appId + "] in net-zone [" + netZoneId + "]. ");
		...
		if (residueCpuCount < 0)
			throw new ServiceException(" Application of CPU resources is not enough! ");
		if (residueMemSize < 0)
			throw new ServiceException(" Application of memory resources is not enough! ");
		if (residueDiskSize < 0)
			throw new ServiceException(" Application of storage resources is not enough! ");
	...
	public int getAppImageCount(Long appId) {
		CPcAppImage cdt = new CPcAppImage();
		cdt.setAppId(appId);
		List<PcAppImage> list = queryList(cdt, null);
